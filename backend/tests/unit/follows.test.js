...
describe("POST /follows/:userId", () => {
  ...
  it("returns 400 when trying to follow yourself", async () => {
    const res = await request(app).post("/follows/1").set(authHeader({ id: 1, username: "alice" }));
    expect(res.status).toBe(400);
  });

  it("returns 404 when target user does not exist", async () => {
    db.user.findUnique.mockResolvedValue(null);
    const res = await request(app).post("/follows/9999").set(authHeader());
    expect(res.status).toBe(404);
  });

  it("returns 401 when not authenticated", async () => {
    const res = await request(app).post("/follows/2");
...
describe("DELETE /follows/:userId", () => {
  it("unfollows a user", async () => {
    ...
    const res = await request(app).delete("/follows/2").set(authHeader());
...
describe("PATCH /follows/:userId", () => {
  ...
  it("returns 400 when favorited field is missing", async () => {
    const res = await request(app).patch("/follows/2").set(authHeader()).send({});
    expect(res.status).toBe(400);
  });
});
